...
There is no nested `PaperTree` on the V1 result. `PaperStructureTree` is an independently versioned paper-artifact binding of the shared `StructureTree` base, linked to an exact source and input chunk set; see [Build and retrieve from a page-preserving structure tree](../mind/retrieval.md).
