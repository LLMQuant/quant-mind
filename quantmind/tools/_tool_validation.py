...
def validate_tool_attributes(cls, check_imports: bool = True) -> None:
    ...
    0. Any argument of __init__ should have a default.
        Args chosen at init are not traceable, so we cannot rebuild the source
        code for them, thus any important arg should be defined as a class
        attribute.
    1. About the class:
        ...
        def _check_init_function_parameters(self, node):
            ...
            for arg, default in reversed(
                ...
                if default is None:
                    ...
                    default, (ast.Constant, ast.Dict, ast.List, ast.Set)
                ):
    ...
    for node in class_node.body:
        if isinstance(node, ast.FunctionDef):
            ...
            errors += [
                f"- {node.name}: {error}" for error in method_checker.errors
            ]
