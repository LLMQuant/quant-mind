...
def get_imports(code: str) -> list[str]:
    ...
    # filter out imports under is_flash_attn_2_available block for avoid import issues in cpu
    # only environment
    code = re.sub(
    ...
    imports += re.findall(
        r"^\s*from\s+(\S+)\s+import", code, flags=re.MULTILINE
    )
...
def get_json_schema(func: Callable) -> dict:
    """Generate a JSON schema for a function based on its docstring and type hints.

    This is mostly used for passing lists of tools to a chat template. The JSON
    schema contains the name and description of the function, as well as the
    names, types and descriptions for each of its arguments.
    `get_json_schema()` requires that the function has a docstring, and that
    each argument has a description in the docstring, in the standard Google
    docstring format shown below. It also requires that all the function
    arguments have a valid Python type hint.

    Although it is not required, a `Returns` block can also be added, which
    will be included in the schema. This is optional because most chat
    templates ignore the return value of the function.
    ...
    The general use for these schemas is that they are used to generate tool descriptions for chat
    templates that support them, like so:
    ...
    `(choices: ["tea", "coffee"])`, which will be parsed into an `enum` field in the schema.
    Note that this will only be parsed correctly if it is at the end of the line:
    ...
    if (
        return_dict := json_schema["properties"].pop("return", None)
    ) is not None:
        if (
    ...
    for arg, schema in json_schema["properties"].items():
        ...
        if enum_choices:
            schema["enum"] = [
                c.strip() for c in json.loads(enum_choices.group(1))
            ]
...
# Extracts the Returns: block from the docstring, if present. Note that most chat templates ignore
# the return type/doc!
...
def _parse_google_format_docstring(
    ...
    Parses a Google-style docstring to extract the function description, argument descriptions,
...
def _convert_type_hints_to_json_schema(
    ...
    for param_name, param in signature.parameters.items():
        if (
            ...
            and error_on_missing_type_hints
        ):
...
def _parse_type_hint(hint: type) -> dict:
    ...
    if origin is None:
        ...
        return {
            "type": "array",
            "prefixItems": [_parse_type_hint(t) for t in args],
        }
...
def _parse_union_type(args: tuple[Any, ...]) -> dict:
    ...
    if len(subtypes) == 1:
        ...
        return_dict = {
            "type": sorted([subtype["type"] for subtype in subtypes])
        }
    else:
