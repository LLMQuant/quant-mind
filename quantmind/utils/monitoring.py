...
from .agentic_ext import escape_code_brackets
...
class Timing:
    ...
    def duration(self):
        return (
            None if self.end_time is None else self.end_time - self.start_time
        )
...
class AgentLogger:
    ...
    def __init__(
        self, level: LogLevel = LogLevel.INFO, console: Console | None = None
    ):
        self.level = level
        if console is None:
    ...
    def log(  # noqa: D417
        self, *args, level: int | str | LogLevel = LogLevel.INFO, **kwargs
    ) -> None:
    ...
    def visualize_agent_tree(self, agent):
        def create_tools_section(tools_dict):
            ...
            for name, tool in tools_dict.items():
                ...
                table.add_row(
                    name,
                    getattr(tool, "description", str(tool)),
                    "\n".join(args),
                )
        ...
        def build_agent_tree(parent_tree, agent_obj):
            ...
            if agent_obj.managed_agents:
                agents_branch = parent_tree.add(
                    "🤖 [italic #1E90FF]Managed agents:"
                )
