...
from quantmind.tools._function_type_hints_utils import (
...
@pytest.fixture
def complex_types_func():  # noqa: D103
    def process_data(
        items: list[str], config: dict[str, float], point: tuple[int, int]
    ) -> dict:
        """Process some data.
...
@pytest.fixture
def optional_types_func():  # noqa: D103
    def process_with_optional(
        required_arg: str, optional_arg: int | None = None
    ) -> str:
...
@pytest.fixture
def enum_choices_func():  # noqa: D103
...
@pytest.fixture
def union_types_func():  # noqa: D103
...
@pytest.fixture
def nested_types_func():  # noqa: D103
...
@pytest.fixture
def typed_docstring_func():  # noqa: D103
...
@pytest.fixture
def mismatched_types_func():  # noqa: D103
...
@pytest.fixture
def complex_docstring_types_func():  # noqa: D103
...
@pytest.fixture
def keywords_in_description_func():  # noqa: D103
...
class TestGetJsonSchema:
    """Test for get_json_schema function."""
    ...
    def test_get_json_schema_example(self):
        ...
        assert (
            schema["function"]["parameters"]["properties"]["y"]
            == expected_schema["parameters"]["properties"]["y"]
        )
    ...
    def test_typed_docstring_parsing(self, typed_docstring_func):
        ...
        assert (
            schema["function"]["parameters"]["properties"]["y"]["type"]
            == "number"
        )
        ...
        assert (
            schema["function"]["return"]["description"]
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                "typed_docstring_func",
                "An integer parameter with type in docstring.",
            ),
            ("complex_docstring_types_func", "Nested structure with types."),
        ],
    )
    def test_type_in_description_handling(
        self, request, fixture_name, expected_description
    ):
        ...
        first_param_name = list(
            schema["function"]["parameters"]["properties"].keys()
        )[0]
        assert (
            schema["function"]["parameters"]["properties"][first_param_name][
                "description"
            ]
    ...
    def test_with_special_words_in_description_func(
        self, keywords_in_description_func
    ):
